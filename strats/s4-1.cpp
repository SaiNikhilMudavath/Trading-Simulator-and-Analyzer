...
void mrt(double threshold,double stop_loss_threshold)
{
    //reading values1
    std::ifstream file1("temp1.csv");
    if (!file1.is_open())
    {
        std:cerr<<"failed to open the file1. "<<endl;
    }
    string line;
    vector<string> temp1;
    ...
    vector<double> values1=strtodouble(temp1);vector<double> values2=strtodouble(temp2);
    int x1=values1.back();values1.pop_back();int n1=values1.back();values1.pop_back();
    int x2=values2.back();values2.pop_back();int n2=values2.back();values2.pop_back();
    double x1_counter=0;double x2_counter=0;
    ...
    for(int i=0;i<=spread.size()-n1;i++) //because n1 and n2 are same.
    {
        ...
        double r_mean=a[0];double r_sd=a[1];
        double z_score=((spread[i+n1-1]-r_mean)/r_sd);

        if((((z_score>threshold) && (z_score<stop_loss_threshold))||(z_score<(-1.0)*stop_loss_threshold))&&(x1_counter>(-1.0)*x1)&&(x2_counter<x2))
        {
            //sell s1 and buy s2
        ...
        else if(((z_score<((-1.0)*threshold)&&(z_score>(-1.0)*stop_loss_threshold))||(z_score>stop_loss_threshold))&&(x1_counter<x1)&&(x2_counter>(-1.0)*x2))
        {
            //buy s1 and sell s2
        ...
        file3<<temp1[i+n1-1].substr(0,11)<<std::fixed<<setprecision(10)<<running_sum_1<<endl;
        if(i==spread.size()-n1)
        {
            double a=running_sum_1+(x1_counter*values1[i+n1-1])+(x2_counter*values2[i+n2-1]);
            file6<<std::fixed<<std::setprecision(10)<<a<<endl;
        }
    }
}
int main(int argc, char *argv[])
{
    mrt(stod(argv[1]),stod(argv[2]));
}